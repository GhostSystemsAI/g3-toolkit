...
- **Demo (run it):** the dev-server scenarios run from `pnpm dev`, which
  opens a gallery you pick from: four domain shells (Biomedical,
  Auditor, MBSE, Supply Chain) and the capability surfaces (Analytics,
  Style Lab, Routing Lab, RDF 1.2 Hyperarcs, Legibility Lab, Ontology
  Workbench; Scale is deployment-only). Analytics is also importable
  standalone from
  [`examples/decision-dashboards`](../examples/decision-dashboards);
  Workspace is in [`examples/full-workspace`](../examples/full-workspace).
  The Schema Dashboard was retired 2026-07-07 with its matrix and
  sankey folded into Analytics, and the Flagship demo was retired
  2026-07-03; rows below name Analytics or Ontology Workbench where they
  used to name those two.
...
| Show the same data as a table, tree, matrix, timeline, map, or Sankey                  | Biomedical (tree, table); Supply Chain (map); Auditor (timeline); Analytics (matrix, Sankey) | Composition levels                                                                                     | Views                                                                             |
...
| Lay out a structural / SysML block view (typed compartments, ports)                    | MBSE; Ontology Workbench                                                                              | Lay out a structural (UML-style) view                                                                  | Atoms / SpecPort                                                                  |
| Validate against SHACL and show the report (graph overlays + shape browser)            | Biomedical; Auditor; Ontology Workbench                                                               | Visualize a SHACL validation report; Render a SHACL shape graph; Link the shape view and the data view | Reference / Charts (Shacl Shape Browser); Views (Diff)                            |
| Show provenance and time (PROV-O chain, timeline, temporal filter)                     | Auditor                                                                                   | (no dedicated recipe yet)                                                                              | Views (TimelineView, on `@g3t/react/timeline`); Molecules / TemporalRangeFilter    |
| Project and clean the graph (collapse blank nodes and literals; diff before/after)     | Biomedical; Analytics; RDF 1.2 Hyperarcs                                                             | (no dedicated recipe yet; ProjectionPipeline is a core API)                                            | Views (DiffRenderer)                                                              |
...
| Build a query interactively                                                            | Analytics; Ontology Workbench                                                                         | The integration surface                                                                                | Views (QueryEditor)                                                               |
...
| Route edges around nodes instead of through them                                       | Routing Lab; Ontology Workbench; Auditor                                                  | Route edges around nodes on any layout                                                                 | (canvas behavior; the Routing Lab grades it live)                                 |
| Make a dense "hairball" readable (bundle edges, spread hubs, collapse fan-ins)          | Legibility Lab; Scale                                                                     | Edge bundling (dense-scene legibility); Scaling: collapse large graphs to clusters                      | (canvas behavior; the Legibility Lab is the didactic view)                        |
| Show RDF 1.2 triple terms (annotate a statement with who said it, confidence, time)    | RDF 1.2 Hyperarcs                                                                         | RDF 1.2 hyperarcs (triple terms)                                                                       | (projection output; the shell toggles hyperarc against edge render)               |
| Export a PNG snapshot of the canvas                                                    | all scenarios (toolbar Export menu)                                                       | Image export (PNG snapshot)                                                                            | UX Surface / Toolbar                                                              |
