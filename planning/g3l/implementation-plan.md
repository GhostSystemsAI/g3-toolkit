...
- **A1 (P0).** Execute the recorded-but-unshipped 12.20 experiment: on the collapse/expand same-graph-rebuild path, feed surviving nodes' prior coordinates as `elk.position` hints and switch the three ELK strategies to INTERACTIVE for that run only. SHIPPED as the `sketch` option on `StructuralLayoutOptions` (G3L:LAY-017). The `crossingMinimization: "INTERACTIVE"` seam this item originally pointed at no longer exists: that option was one of five removed 2026-08-15 for reaching no engine. Sketch mode is not affected, since the g3t engine reads `options.sketch` directly. Accept criterion is spec LAY-018 verbatim: untouched containers move less than one grid unit; the toggled container resizes in place.
